...
function printHelp() {
  echo "Usage: "
  echo "  cyfn.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-o <consensus-type>] [-n <cc_name>] -v [<cc_version>] [-a] [-w]"
  ...
  echo "      - 'cc'  - install chaincode"
  echo "      - 'channel'  - create channel-artifacts"
  echo "      - 'join'  - create channel-artifacts and join a channel"
  echo "    -c <channel name> - channel name to use (defaults to \"mychannel\")"
  echo "    -t <timeout> - CLI timeout duration in seconds (defaults to 10)"
  echo "    -d <delay> - delay duration in seconds (defaults to 3)"
  echo "    -f <docker-compose-file> - specify which docker-compose file use (defaults to docker-compose-cli.yaml)"
  echo "    -s <dbtype> - the database backend to use: goleveldb (default) or couchdb"
  echo "    -p <peer> - the number of Peer to execute a command"
  echo "    -r <org> - the number of Org to execute a command"
#  echo "    -l <language> - the chaincode language: golang (default) or node"
  echo "    -o <consensus-type> - the consensus-type of the ordering service: solo (default), kafka, or etcdraft"
  echo "    -n <cc_name> - the name of the chaincode"
  echo "    -v <cc_version> - the version of the chaincode"
#  echo "    -i <imagetag> - the tag to be used to launch the network (defaults to \"latest\")"
  echo "    -a - launch certificate authorities (no certificate authorities are launched by default)"
  echo "    -w - do not deploy chaincode (abstore chaincode is deployed by default)"
#  echo "    -v - verbose mode"
  echo "  cyfn.sh -h (print this message)"
  echo
  echo "Typically, one would first generate the required certificates and "
  echo "genesis block, then bring up the network. e.g.:"
  echo
  echo "	cyfn.sh generate -c mychannel"
  echo "	cyfn.sh up -c mychannel -s couchdb"
  echo "	cyfn.sh down -c mychannel"
  echo "        cyfn.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	cyfn.sh generate"
  echo "	cyfn.sh up"
  echo "	cyfn.sh down"
}
...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.'$CC_NAME'.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.'$CC_NAME'.*/) {print $3}')
...
function installCC() {
  ...
  docker exec cli scripts/installCC.sh $PEER $ORG $CC_NAME $CC_VERSION
  if [ $? -ne 0 ]; then
